...
public class RegistrationInfo {
	...
	public void register() {
		...
		try {
			doRegister(getDeviceRegistrationId(), "ac2dm", getDeviceId(),
					getAccountName());
		} catch (Exception e) {
	...
	public String toString() {
		return "RegistrationInfo [accountName=" + accountName + ", deviceId="
				+ deviceId + ", deviceRegistrationId=" + deviceRegistrationId
				+ "]";
	}
	...
	private void doRegister(String deviceRegistrationId, String deviceType,
			String deviceId, String accountName) throws Exception {
		log.info("in register: accountName = " + accountName);
		PersistenceManager pm = PMF.get().getPersistenceManager();
		try {
			List<DeviceInfo> registrations = DeviceInfo
					.getDeviceInfoForUser(accountName);
