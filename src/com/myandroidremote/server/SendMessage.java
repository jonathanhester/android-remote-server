...
public class SendMessage {
	...
	public static String sendMessage(ServletContext context, String recipient,
		...
		try {
			...
			String collapseKey = "" + androidCommand.getId();
			...
			for (DeviceInfo deviceInfo : registrations) {
				...
				res = doSendViaC2dm(androidCommand, push, collapseKey,
						deviceInfo);
				numSendAttempts++;
	...
	private static boolean doSendViaC2dm(AndroidCommand androidCommand, C2DMessaging push,
			String collapseKey, DeviceInfo deviceInfo) {

		return push.sendNoRetry(deviceInfo.getDeviceRegistrationID(),
				collapseKey, RemoteCommands.PARAM_COMMAND,
				Integer.toString(androidCommand.getCommand()), RemoteCommands.PARAM_COMMAND_ID,
						Long.toString(androidCommand.getId()));
	}
}
